...
| --- | --- | --- |
| **Capacitor** (chosen) | Native iOS and Android shells around `public/`. `appId app.redletter.quietpage`. | Reuses the working build and its 56 tests and 13 browser walks. Web Share, Canvas proofs, the service worker and the Press need no port. First build is minutes, not weeks of parity work. |
