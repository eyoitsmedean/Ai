...
The only thing that changes per project. Put it in the project's files (or `CLAUDE.md`), or paste it under the kickoff message. Every line the brief answers is a question the bot never has to ask; every choice left blank is one the bot will make itself and label ASSUMED; every fact left blank is one it will ask for or parameterize — never invent.
...
Briefs that live in this repo:

- `CLAUDE.md` — Red Letter (the Advisor build), corrected against the repo on 2026-09-06.
- `folio/BRIEF.md` — Ninety Days · One Plan.

### Filled example — as Dean wrote it on 2026-09-06 (superseded by `CLAUDE.md`, which corrects what has moved)
...
The finished thing: a working app that builds for iPhone and Android; code in the repo (github.com/eyoitsmedean/Ai) with tests, prompts, and a written evaluation set with results.
...
Capabilities this bot has: web search, code execution, file output, subagents
Budget and size: MASTERWORK research budget; multi-session build — end every session with a STATE block
Hard constraints: scope is Jesus's words only, not the whole Bible; mobile-first performance; the Bible translation must be public domain or licensed for app distribution — state which, with a source
Decisions already made: the name "The Red Letter Advisor"; the parchment-and-crimson palette; advisor-first product, not a scholarship tool — all recorded in the repo's CLAUDE.md. Not yet decided: the mobile stack — propose two options in ANCHOR with tradeoffs and default to the one that reuses the existing HTML build unless I say otherwise.
Facts only I have: none outstanding — ask if you hit one.
Assets, repos, and system of record: the repo; the standalone HTML build on Claude's API; CLAUDE.md is the system of record for decisions.
Known risks: product intent was inverted once (scholarship-first) — read CLAUDE.md before designing anything; unverified checks cannot ship.
Authority: edit code and branches freely; no App Store / Play submission, no production deploy, no spending without sign-off.
Quality reference: <Dean to name two apps> — what to match: time from first open to first useful answer, and warmth of tone.
```
...
For platforms with short instruction fields. Same spine, fewer words; under 8,000 characters. The copy that Cursor loads on every turn lives at `.cursor/rules/atelier-protocol.mdc`.
...
*The appendix's VERIFIED labels are Dean's, from the session that wrote the protocol; this file reproduces them and did not re-open those sources.*
