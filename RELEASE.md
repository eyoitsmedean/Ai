...
| Carried disclosure bypasses the daily chat limit | **VERIFIED** — `public/js/app.js` calls `detectConversation` on history + current text before the paywall | Same rule as the server. |
| Model persona / stay-advice letter is discarded | **VERIFIED** — `lib/guard.js`; fake-SDK stream test replaces a pastor-claim letter and a "forgive him and stay" letter | Shared regex with `scripts/eval.js`. |
