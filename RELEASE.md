...
Every line is marked **verified** (run in this environment, with what was run) or **unverified** (what it would take). Nothing below is described as passed that was not run. Last run: 2026-09-14 — ATELIER; `npm test` 80/80; eval 113/113 both composers; `npm run sit` 8/8; `npm run qa` 22/22 against `http://127.0.0.1:3001`.
...
| Every Advisor quote is exact KJV speech for its citation, on the server composer and the static-hosting composer | verified | `npm run eval` gate `exact`, 113/113 both; `test/scripture.test.js` |
| The static page loads the same composer the tests and eval load — `public/data/advisor.js`, `curated.js`, `paths.js`, `signals.js`, `rooms.js` are the only copies | verified | the `data/` duplicates are deleted; `scripts/eval.js --client` and `test/eval.test.js` read `public/data/`; `npm run qa` — 'the served composer hears violence, poisoning and grief the way the server does' |
...
| Only His part of a verse where someone else also speaks is kept (John 12:28 keeps 'Father, glorify thy name', not the voice from heaven; Mark 8:19, Luke 20:16, John 6:64, 12:36, 19:27); His Aramaic keeps the evangelist's gloss; Luke 2:49 and the safe omitted class are in the live corpus | verified | `SPOKEN_OVERRIDES` (15), `SPOKEN_ADDITIONS` (23); `test/scripture.test.js` — 'keeps only His part of a verse…' |
...
| Every quotation the page itself can show (rooms, daily words, Seven Days, and the Forty data list) is His exact words. Forty is not a UI path yet | verified | `test/eval.test.js` — 'every quotation the page can show' |
...
| No letter's prose contains the platitudes that wound — 'God is punishing', 'lack of faith', 'happens for a reason', 'better place', 'you will never feel', 'pray harder' (36 phrases) | verified | eval `scope` gate `GLOBAL_FORBID`, 113/113 both composers; the model prompt now forbids them in words as well |
...
| Item | Status | Evidence |
| --- | --- | --- |
| Crisis-shaped questions get 988 + findahelpline before any scripture, and only crisis-safe passages; the notice names chat at 988lifeline.org | verified | eval `crisis` gate, 26/26 crisis inputs on both composers; `test/eval.test.js` asserts the chat URL; numbers VERIFIED 2026-09-11 at 988lifeline.org |
...
| The first passage a person reads belongs to the room they named; a veteran is not handed 'take no thought for the morrow', a betrayed wife is not handed 'if ye forgive not', a parent is not handed 'love your enemies' about her daughter, the bereaved and the elderly are not handed 'many mansions', the bullied child is not asked to forgive today, the body not getting better is not told about faith, a job lost is worry and a house lost is suffering, not Purpose; the one who hit and the veteran open on Luke 15:4 on **both** composers; affair-day opens on Matthew 11:28 on both | verified | eval `theme` gate, 113/113 both composers; `test/eval.test.js` first-citation identity on `danger-05`, `life-27`, `life-44`, `life-36`; all twelve rooms share a first passage across `lib/curated.js`, `public/data/curated.js`, and `public/curated.json` |
| Ordinary words route by one `guessThemes` on both composers: 'hit me up' is money not a wound; 'I hit my head' is not Shame; IVF / a baby shower is Grief; talking to the ceiling is Faith | verified | eval 113/113 both composers; first-cite identity on `life-08` John 20:29, `life-10` Luke 15:4, `life-31` Matthew 5:4, `edge-08` not Shame, `edge-15` Matthew 6:34, `forgery-05` Luke 12:32; `public/data/rooms.js` |
...
| Item | Status | Evidence |
| --- | --- | --- |
| Unit tests | verified | `npm test` — 80 pass (includes both evals, Today identity, help/atlas, rooms.js, first-citation identity) |
| Live HTTP smoke | verified | `npm run smoke` — 10 checks |
| First-session browser walk (title page, lectio, journal, Seek, Advisor, crisis, poison interrupt, library, blessing, desktop rail, Leave, named-map colophon, the served composer with no server behind it) | verified | `npm run qa` — 22 checks, headless Chrome 390×844 and 1100×800 against port 3001 |
| Offline: precache complete (now including `data/rooms.js`, cache `rla-v20`), Today renders, blessing link opens from cache | verified | `test/eval.test.js` asserts `rooms.js` and `signals.js` are precached; help.html still leads the list |
