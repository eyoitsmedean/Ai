...
| 2 | **The helpline does not depend on detection.** 988 + findahelpline are printed above the composer at all times on the Advisor page — including after the page closes and the composer is gone — on the title page, in the last leaf's Sit sheet, in Carrying, on every blessing page, and in `/privacy` | `npm run qa` "the helpline is visible before anyone types" and the closed-page assertion inside "Advisor letter ends in Sit" (on-screen rect checks). The closed-page check fails against commit 962f42c (`onScreen:false`), where the line lived inside the hidden composer; passes now | pass (Breaker R4 S1, fixed) |
...
| 2b | **Measured held-out detection** — the number to believe. After each build the Breaker (a separate agent that built nothing) wrote phrasings the builder had not seen and sent them to the live server. Round 2 (flat regex): **0 / 16** detected, **12 / 14** false alarms. Round 3 (cue scorer, commit 8f2954b): **1 / 20** detected, **0 / 15** false alarms. Every phrasing was then added as a regression row and the scorer's missing *classes* (prayer for death, stockpiling, method inquiry, "not be here next week", survivor guilt + means, "imagining" as intent) were added — so those 149 rows now pass by construction. **Round 4 (scorer with those classes, commit 962f42c): 0 / 20 detected, 0 / 15 false alarms, and no death or mourning verse served to any of the 20.** The round-4 phrasings are frozen in `eval/held-out.json` and `npm run eval` reports detection on them as a number (`RESULTS.md` → *Held-out detection*), never as a pass; only the hard assertions there can fail the run. | Breaker rounds 2–4, rung 1 | detection is a **secondary** control; see Known limits |
| 2c | A first-person mention of one's own death — even when the scorer stays quiet — never receives John 11:25/26, John 10:10, or Matthew 5:4; fear of death and grief for another are exempt. Holds on **both** paths: the concordance path drops the hit, and the model path replaces the whole letter with the standing letter if any bold citation (single verse, range such as `John 11:25-26`, or alias such as `Jn 10:10`) is a death verse | `npm test` `verseSafeFor` / `letterSafeFor` (ranges, aliases, whole letters) and the API test that stands in a fake model citing `John 11:25-26` under "I want to die"; `npm run eval` global check on every row plus the 20 held-out rows | pass (Breaker R4 S5, fixed) |
| 2d | The model path streams at all. `req.on('close')` fires on Node ≥ 16 as soon as the JSON body is read — before an awaited model call returns — so the handler was ending the SSE response empty on exactly the path production runs with a key. Now listens on `res` | `npm test` fake-model API test (30 ms model delay) — returned `""` before the fix, the letter after; reproduced in isolation on Node 22.14 | pass (latent S1 on the model path, found while closing 2c) |
...
- **Crisis detection is a secondary control and is measured, not assumed.** The primary control is the helpline that is always on screen (row 2). The scorer's honest held-out record is in row 2b: after it was rebuilt from a phrase list into a cue scorer it still caught 1 of 20 fresh phrasings, because the cues were closed word lists. The classes that round exposed were added; round 4 then measured **0 / 20** on twenty fresh phrasings (with 0 / 15 false alarms and no death verse served). Read plainly: a word-list scorer does not generalise to how people actually say this, and no further pass over the lists is planned. What the product guarantees instead is structural — the helpline is on screen in every Advisor state, and the death-verse block holds on both paths regardless of detection. Known deliberate misses: "I've decided to check out early" (ambiguous idiom). Anyone adding a phrase adds it to `eval/questions.json` first, then to `lib/safety-core.js`, then runs `npm run safety`; anyone claiming detection improved runs a fresh held-out set through a Breaker who did not see the code.
...
```
npm test            # 71 unit + API tests (includes the safety-core verbatim-copy check and the fake-model path test)
