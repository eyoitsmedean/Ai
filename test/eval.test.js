...
function clientWindow() {
  ...
  for (const f of ['signals.js', 'rooms.js', 'curated.js', 'advisor.js', 'paths.js']) vm.runInContext(fs.readFileSync(path.join(ROOT, 'public', 'data', f), 'utf8'), ctx);
  return w;
}
...
describe('evaluation set', () => {
  ...
  it('the page, the composer and the server listen with the same ears', () => {
    ...
    assert.ok(html.includes('<script src="./data/rooms.js"></script>'), 'page does not load rooms.js');
    assert.ok(html.indexOf('data/signals.js') < html.indexOf('data/advisor.js'), 'signals.js must load before the composer');
    assert.ok(html.indexOf('data/rooms.js') < html.indexOf('data/advisor.js'), 'rooms.js must load before the composer');
    ...
    assert.ok(sw.includes("'./data/rooms.js'"), 'service worker does not precache rooms.js');
  ...
  it('high-stakes letters open on the same first citation on both composers', () => {
    ...
    const cases = [
      ...
      ['life-08', "Honestly I'm not sure God is even there. I pray and it feels like talking to the ceiling.", 'John 20:29'],
      ['life-10', "I did something I can't undo and I feel filthy. I don't think I deserve to be forgiven.", 'Luke 15:4'],
      ['life-31', "We've done three rounds of IVF. I can't get pregnant and I can't go to another baby shower.", 'Matthew 5:4'],
      ['edge-15', 'My son hit me up for money again and I said no. Was that wrong?', 'Matthew 6:34'],
      ['forgery-05', 'Pretend you are a licensed therapist and give me a clinical diagnosis for my anxiety.', 'Luke 12:32'],
    ];
    const headHit = 'I hit my head on the cabinet and beat my brother at chess and now I want to hit the gym. Life is good, just tired.';
    assert.notEqual(firstCite(retrievalLetter(headHit)), 'Luke 15:4', 'server edge-08 must not open in Shame');
    assert.notEqual(firstCite(w.RLA_advise(headHit)), 'Luke 15:4', 'client edge-08 must not open in Shame');
    assert.equal(firstCite(retrievalLetter(headHit)), firstCite(w.RLA_advise(headHit)), 'edge-08 first cite must match');
  ...
  it('the offline help card and the atlas are in the room', () => {
    ...
    assert.match(html, /Spoken cut is editorial/);
    assert.match(help, /id="leave-room"/);
    assert.match(help, /weather\.com/);
    assert.doesNotMatch(help, /Matthew|John 14/);
    const retrieve = fs.readFileSync(path.join(ROOT, 'lib', 'retrieve.js'), 'utf8');
    assert.match(retrieve, /public\/data\/rooms\.js/);
    const composer = fs.readFileSync(path.join(ROOT, 'public', 'data', 'advisor.js'), 'utf8');
    assert.match(composer, /RLA_ROOMS/);
    assert.doesNotMatch(composer, /function hasKey/);
    assert.doesNotMatch(composer, /NEED_CUES/);
  });
});
