...
## 3. How to use
1. `docs/lodestar/CHARTER.md` (the star).  
2. `npm start` → ask “My boss wants me to lie.”  
3. `/?night=1` for the latch.  
4. `?fresh=1` → Turn the page → To ask → one sentence.  
5. `docs/lodestar/INDEX.md` for the rest.
...
C6 as a named goal is reversible. Night = 21:00–06:00 local or `?night=1`. KJV stays the only corpus. Parallel-agent PRs remain unmerged.

## 6. Authorization Requests
1. Letter to Cambridge before any UK paid tier (do not send from here).  
2. Production API host + `ANTHROPIC_API_KEY` so model-path eval can run.  
3. No App Store / Play / production deploy / spend.

## 7. Recruitment Orders
- **Model-path reviewer** — mission: `ANTHROPIC_API_KEY=… npm run eval`; read tone column as the writer. Input: this repo. Output: marked RESULTS.md. Bearing: C2 C4.  
- **Video scout** — no transcript access here. Query: BibleProject “Mammon”; SAMHSA 988 active-rescue. Extract definitions only. Bearing: C1 C3.  
- **Cambridge correspondent** — draft (do not send) the permissions letter: PWA + shells, ~1,900 AV verses, some paid, US entity. Bearing: C5.

## 8. Sources
See each dossier. Klopsch: Crossway, ABS. John 3: Reading Acts, TIPS. APA PDF 2025. Illinois HB 1806. USCCB WICFH. NDVH. 988. Techdirt 2026-05-06. FMG-Bench. JaleesBench. CofE Compline. Lectio365. eBible WEB copyright. BYU / Wikisource on Blayney.
...
## 12. Studio Notes
In the Navigator’s closing message.

## 13. The Ask
Keep C6 as a numbered goal — yes or no?
