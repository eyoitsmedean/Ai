...
| 2026-09-14 | Built `folio/tonight.html`: one page that is the whole 20-min path (window gate → $0 → pick one lane → five gates → one ask on HOLD → keep calc → week close → one receipt), collapsing the seven rooms into a single scroll per Prompt 1–2 ("beautiful, easy, wow, little assembly, not another shelf"). Non-destructive (only added an "open Tonight" link on decision-night.html and index.html). No money-lock reversed; all numbers = Operator Card v2. HOLD-lock unlocks only on window Confirmed/Not needed + lane conditions + Yes (Lamp also needs 5/5 gates Clear). | VERIFIED (11/11 lock-table cases; keep math reproduces locked examples; 390px browser walkthrough) | folio/tonight.html; links on folio/decision-night.html, folio/index.html; branch cursor/tonight-cockpit-4efd, PR eyoitsmedean/Ai#54 |
