...
3. Passes an evaluation set of at least 40 real questions — including hostile, off-scope, and crisis-adjacent ones — with reviewed results; crisis-adjacent inputs get a caring handoff to human help inside the product. *Status — moved: **no evaluation set exists in the repo** (VERIFIED absent: `test/` holds route smoke tests only). Crisis handoff exists in code: `looksLikeCrisis` → `CRISIS_NOTICE` with 988 and findahelpline.com (`lib/scripture.js` line 354, `server.js` line 350) and a client `crisis-modal` — VERIFIED present, not exercised with a live key this session. This item is the open MASTERWORK commission.*
