...
| D11 | The evaluation set (`eval/questions.json`, 96 questions) and the rendered UI check (`scripts/ui-check.js`) run in CI; a regression in any category blocks the merge. | settled | 2026-09-14 (count) |
...
- 2026-09-14 — First-ten loop: a carried word now returns to empty Advisor (sitting-with + chip); `/share` Hear/Copy/Carry is tap-only (no autoplay). Library Carry stays on Advisor (C4). After-path Carry uses the last saying, and each verse block can Carry its own.
