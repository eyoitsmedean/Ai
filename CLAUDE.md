...
| One crisis detector, byte-identical in server, folio page, on-device fallback, and `/ask`; a test fails on drift | locked 2026-09-06, extended 2026-09-11 | `test/advise.test.js` |
...
| Intended product shape is **one screen** at `/ask` (Ask → The words → Meaning ≤4 lines → What this bot cannot do). The five-room folio is the atelier, not a launch | locked 2026-09-11 (later amendment) | `public/ask.html`, `lib/ask.js`, `docs/CANON.md` |
| Public publish, store submit, and GitHub Pages-as-launch remain **WATCH / HOLD** | locked 2026-09-11 | Notion one-screen page; `LAUNCH.md` parked |
| Free US advisor texts: WEB (PD text; name is trademark) or KJV-US. Do not paste NIV, ESV, NASB, NRSV, CSB, The Message. Dean records the chosen text before any public URL | locked 2026-09-11 | `docs/RESEARCH.md` |
| Crisis and abuse paths **stop** on `/ask` and in folio letters — name a human hotline, no Gospel counsel after 988. Soft-crisis (someone who says they are not suicidal) still cites on the folio and stops on `/ask` | locked 2026-09-11 on `/ask`; folio letters closed 2026-09-14 | `lib/ask.js`; `lib/advise.js` `crisisLetter` et al.; `eval/questions.json` |
| Mobile: **Capacitor** wrapping the existing HTML build (`appId app.redletter.quietpage`). Allowed as a local shell. **Retired as a launch / store path** | locked 2026-09-02, amended 2026-09-11 | `capacitor.config.json`, `android/`, `MOBILE.md` |
...
| `/ask` evaluation set, stop vs cite (13 items, including short Spanish, mixed IPV, and IPV after a crisis line) | verified 2026-09-14 | `npm run eval:ask` → `eval/RESULTS-ask.md` |
| Same 13 `/ask` items against on-device `composeAsk` (`public/data/advisor.js`) | verified 2026-09-14 | `npm run eval:ask:device` → `eval/RESULTS-ask-device.md` |
| Advisor evaluation set, **on-device composer** (`public/data/advisor.js`, generated from `lib/advise.js`): 82/82, 63 distinct letters | verified 2026-09-14 | `npm run eval:device` → `eval/RESULTS-device.md` |
...
| Browser QA (18 walks: Press, Forty, Advisor chips, daily gate, `/ask` crisis stop + no Google Fonts, `/gate`, `/letter`, `/make`) | verified 2026-09-19 | `npm start` then `npm run qa` |
...
```bash
npm install
npm start          # http://localhost:3000/ask — one-screen product; / is the atelier folio
npm test           # Node suite
npm run eval:ask   # one-screen stop-vs-cite set → eval/RESULTS-ask.md
npm run eval:ask:device # same set against on-device composeAsk
...
- **2026-09-11 — Sprint: one composer everywhere.** On-device Advisor is generated from `lib/advise.js` (`npm run advisor`); Pages/offline no longer get the keyword stub. Evangelist frames stripped from the spoken library (47 of 49; two kept as His own narration). Matthew 27:46 and Mark 15:34 carry the KJV English. Advisor chips now exercise shame, grief, a named parable, a brought verse, and negation; follow-up chips continue a letter. Suite 64, eval 82/82, device eval 82/82, browser 15/15.
- **2026-09-19 — Saying maker.** `/make` turns a sealed saying into a quiet motif (A minor pentatonic, Web Audio, no autoplay). Crisis and abuse never become music. Not a hymn. Not AEVUM. WATCH holds. Second pass: phrase breaths, cadence on E/A, pitch staff, word-following Play, Sit/Mid/Lift, optional fifth. `/letter` links the same motif.
- **2026-09-14 — Folio stop.** Crisis, concern, loss, and abuse letters in `lib/advise.js` no longer quote after 988. Eval 82 encodes `cite: false` on those paths. Soft-crisis (hope-1) still cites on the folio. `/ask` footer no longer warns that the atelier quotes after the number. WATCH holds.
- **2026-09-14 — First useful answer.** `/ask` first paint is the sealed tired counsel (John 16:33), matching `composeAsk('I am so tired')`, plus Print this saying. WEB Matthew 11:28 remains the labeled other-text footnote. Suite 80.
- **2026-09-14 — ATELIER.** On-device `composeAsk` (same bundle as the folio composer). `/ask` uses it when `/api/ask` is down and still records prior. `/letter` falls back to `RLA_lookup` (His speech only). RESEARCH.md Yale URL/2039 corrected. Folio crisis letters still quote after 988. Suite 80, eval:ask 13/13, eval:ask:device 13/13, folio eval 82/82.
- **2026-09-14 — LODESTAR Cycle 1.** Intent Charter in `docs/lodestar/`. `/ask` eval (13). Crisis voice: kind-split, `abuseCrisis`, short Spanish AYUDA, no Google Fonts on `/ask`. `/gate` scratch pad. `/letter` printable saying + colophon. Folio crisis letters still quote after 988. Suite 78, eval:ask 13/13, folio eval 82/82.
- **2026-09-11 — Recovery commission.** First ten prompts of this conversation recovered from the run transcript (not memory). Later Notion amendments win: WATCH, folio stays paper, one screen, WEB/KJV-US human gate, crisis stop. Flagships: `docs/CANON.md`, `docs/RESEARCH.md`, `/ask` (`lib/ask.js`, `public/ask.html`). Folio crisis letters left intact so eval 82 stays honest. Grok F1–F3 not touched.
