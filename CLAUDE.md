...
- **The helpline is the primary safety control; detection is secondary.** 988 + findahelpline are printed above the Advisor composer at all times (not only after a match), on the title page, in Carrying, on blessing pages, and in `/privacy`. Reason: the Breaker measured the detector on held-out phrasings at 0/16 (flat regex), 1/20 (first cue scorer) and 0/20 (scorer with the round-3 classes) — a control that depends on recognising the sentence cannot be the only one. RELEASE.md row 2b carries the measured numbers. The helpline is page chrome (`#composer-help` is a sibling of `.composer`, not a child), so it stays on screen after the page closes and the composer is hidden; `npm run qa` asserts this in the closed state. **No further pass over the word lists is planned**: round 4 showed the lists do not generalise, and the guarantees are structural instead (this line, and the death-verse block).
...
- **Death-verse block.** Any first-person mention of one's own death that is not fear or grief for another gets the standing letter — never John 11:25/26, John 10:10, Matthew 5:4 — regardless of whether the scorer fired (`verseSafeFor`). It holds on both paths: the concordance path drops the hit; the model path (`letterSafeFor`) replaces the whole letter with the standing letter if any bold citation — single verse, range (`John 11:25-26`), or alias (`Jn 10:10`) — is a death verse. The model's judgment is not relied on for this.
- **Chat streaming listens on the response, not the request.** On Node ≥ 16 `req` emits `close` once its body is consumed, which on the model path is before the model answers; the old `req.on('close') → res.end()` sent an empty stream on exactly the path production runs with a key. `server.js` now sets a `gone` flag from `res.on('close')`. Tests stand in a fake model through `app.locals.chatClient` (never set in production).
...
- **Evaluation set** lives in `eval/questions.json` (149 rows; crisis rows include every held-out phrasing the Breaker wrote in rounds 2–3, and 40 `ordinary` sentences that must trigger nothing); `npm run eval` writes `eval/RESULTS.md`. Add failing phrasings there before touching a pattern. **Round 4's 35 phrasings are frozen in `eval/held-out.json`** and reported as a detection *number* (0/20 · 0/15 false alarms at this commit), never folded into the pass count; only their hard assertions (sealed quotes, no death verse, no false crisis notice) can fail the run. A claim that detection improved requires a fresh held-out set from an agent that did not see the code.
