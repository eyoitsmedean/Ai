...
class EngineState {
  ...
  EngineState copyWith({
    ...
    return EngineState(
      ...
      showAmen: showAmen ?? this.showAmen,
...
class EngineController extends StateNotifier<EngineState> {
  ...
  Future<void> finishSplash() async {
    ...
    if (engine != null) {
      ...
      state = state.copyWith(engine: engine, splashHold: false);
      await persist();
    } else {
      state = state.copyWith(splashHold: false);
    }
  }

  void openStep() {
    state = state.copyWith(showStep: true, showAmen: false);
  }

  Future<void> commitStep([String? words]) async {
    final engine = state.engine;
    if (engine == null) return;
    engine.commitStep(words);
    state = state.copyWith(engine: engine, showStep: false, showAmen: true);
    await persist();
  }

  void amen() {
    state = state.copyWith(showAmen: false);
  }
...
class _AppShellState extends ConsumerState<AppShell> {
  ...
  Widget build(BuildContext context) {
    ...
    if (state.booting || state.splashHold) {
