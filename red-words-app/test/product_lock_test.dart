...
void main() {
  ...
  test('One Honest Step is the second beat, not a Sit room', () {
    ...
    expect(today, contains('step-field'));
    expect(today, contains('In your own words.'));
    expect(today, contains('Amen'));
    expect(today, isNot(contains('Turn the page')));
    expect(
      File('lib/engine/moment.dart').readAsStringSync(),
      contains("You said you'd"),
    );
  });
}
