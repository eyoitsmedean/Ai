...
void main() {
  ...
  test('a step in the person\'s own words is what tomorrow remembers', () {
    final engine = MomentEngine(pack, const LocalMemory());
    engine.commitStep('sit still before I answer');
    expect(engine.memory.committedStep, 'sit still before I answer');
    expect(engine.continuityLine, "You said you'd sit still before I answer");
  });

  test('continuity is absent until a step is committed', () {
    final engine = MomentEngine(pack, const LocalMemory());
    expect(engine.continuityLine, isNull);
  });
