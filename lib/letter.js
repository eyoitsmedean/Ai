...
const COMMONS = [
  ...
  { verse: 'Matthew 7:7', context: 'Asking is allowed. Seeking is allowed. The door is His to open, and He says it opens.' },
  { verse: 'John 6:37', context: 'Coming is enough. He does not turn away the one who comes.' },
...
function composeLetter(text, { history = [] } = {}) {
  ...
  const firstTimeHere = primary && !(history || []).some((m) => (
    m && m.role === 'assistant' && typeof m.content === 'string' && m.content.startsWith(THEMES[primary].opening)
  ));
