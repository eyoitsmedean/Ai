...
const SPEECH_INTROS = [
  ...
  /^And Jesus answering said unto (?:them|him|her),\s*/i,
  /^Then Jesus answering said unto (?:them|him|her),\s*/i,
  /^Jesus answering said unto (?:them|him|her),\s*/i,
  /^And he began to say unto them,\s*/i,
  /^Jesus answered and said(?: unto (?:them|him|her))?,\s*/i,
];
...
function lookup(ref) {
  ...
  for (let v = parsed.start; v <= parsed.end; v++) {
    ...
    const raw = mapped || extractSpoken(full, cite);
    const text = mapped ? extractSpoken(mapped, cite) : raw;
