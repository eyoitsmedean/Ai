...
function looksLikeCrisis(text) {
  ...
  return /\b(suicid|kill myself|killing myself|end my life|end it all|take my life|want to die|wanna die|self[-\s]?harm|hurt myself|cut myself|cutting myself|hang myself|overdos|don'?t want to live|do not want to live|don'?t want to be here anymore|don'?t want to wake up|better off dead|no reason to live|not worth living|can'?t go on|(?:took|take|swallow(?:ed)?) (?:all )?(?:the |my )?pills|would(?:n'?t| not)? notice if i (?:was gone|disappeared|died))\b/i.test(String(text));
}
...
/* Present danger from another person — not the same door as suicide. "Love your enemies" and
   "turn the other cheek" are His words; they are the wrong first word for someone being hurt
   at home (USCCB, When I Call for Help, opened 2026-09-11). */
function looksLikeAbuse(text) {
  if (!text) return false;
  return /\b(hits me|hit me|beats me|beat me|beating me|punched me|chokes me|strangl|raped me|sexually assault|sexual assault|domestic violence|abusive (?:husband|wife|boyfriend|girlfriend|partner|ex)|(?:husband|wife|boyfriend|girlfriend|partner|ex) (?:hits|hit|beats|beat|abuses|threatens)|won'?t let me leave|locks me in|threatens to (?:kill|hurt) (?:me|the kids|our)|(?:he|she|they) (?:hits|hurts) (?:the |our )?kids|forgive (?:him|her) (?:for )?(?:rap|assault|hitting|beating))\b/i.test(String(text));
}
