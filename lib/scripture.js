...
function looksLikeCrisis(text) {
  ...
  const stripped = String(text).replace(/\b(don'?t|do not)\s+(want to die|wanna die)\b/gi, '');
  return /suicid|kill myself|killing myself|end my life|take my life|want to die|wanna die|self[-\s]?harm|hurt myself|cut myself|hang myself|overdos|don't want to live|do not want to live|better off dead|no reason to live|wish i (?:were|was) dead|planning my death|unalive|\bkms\b|jump off|bought a gun|pistol loaded/i.test(stripped);
}
