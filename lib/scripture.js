...
/* The digit lookbehind keeps "1 John 4:18" (an epistle) from parsing as John 4:18. */
const REF_RE = new RegExp(
  `(?<!\\d)(?<!\\d\\s)\\b(${BOOK_PATTERN})\\s+(\\d{1,3})\\s*:\\s*(\\d{1,3})(?:\\s*[–—\\-]\\s*(\\d{1,3}))?`,
  'gi'
);

/* Anything shaped like a scripture reference from any book, used to strip
   citations the corpus cannot vouch for (epistles, Psalms, deuterocanon). */
const ANY_REF_RE = /^(?:[1-3]\s*)?(?:Song\s+of\s+(?:Solomon|Songs)|[A-Z][a-z]+\.?)\s+\d{1,3}(?:\s*[:.]\s*\d{1,3}(?:\s*[–—-]\s*\d{1,3})?)?\s*$/;
...
function verifyAdvisorText(text) {
  ...
  while (i < lines.length) {
    ...
    if (looksLikeCite) {
      ...
      const canonical = isRedLetter(parsed) ? lookup(parsed) : null;
      if (canonical) {
      ...
      if (bold) {
        // A Gospel reference that is not his speech (narrative, the evangelist,
        // another speaker). Drop the citation and its quote rather than print
        // words he did not say under his name.
        i = quote ? index + 1 : i + 1;
        continue;
      }
    }
    if (bold && ANY_REF_RE.test(probe)) {
      // Scripture from outside the four Gospels. The corpus cannot vouch for
      // it and the product does not quote it; drop citation and quote.
      const { quote, index } = extractQuotedLine(lines, i + 1);
      i = quote ? index + 1 : i + 1;
      continue;
    }
...
/* Human-help gate. Shared with the page via data/crisis.js; tuned in test/crisis.test.js. */
const { looksLikeCrisis } = require('../data/crisis.js');
