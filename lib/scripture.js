...
/**
 * The one gate for anything printed as His speech.
 * A reference is spoken only if the red-letter map marks at least one verse in it;
 * narrator verses (genealogies, prologues, stage directions) are refused even though the KJV has them.
 */
function spokenLookup(ref) {
  const parsed = typeof ref === 'string' ? parseRef(ref) : ref;
  if (!parsed) return null;
  const hasMap = Boolean(loadSpoken().books?.[parsed.book]);
  if (hasMap && !isRedLetter(parsed)) return null;
  const hit = lookup(parsed);
  if (!hit) return null;
  if (hasMap && !hit.redLetter) return null;
  return hit;
}
...
function fillPlaceholders(text) {
  ...
  return String(text).replace(PLACEHOLDER_RE, (_, raw) => {
    const hit = spokenLookup(String(raw).replace(/^["'“”]+|["'“”]+$/g, ''));
    if (!hit) return '';
    return `**${hit.citation}**\n“${hit.text}”`;
  }).replace(/\n{3,}/g, '\n\n');
}
...
function verifyAdvisorText(text) {
  ...
  while (i < lines.length) {
    ...
    if (looksLikeCite) {
      ...
      const canonical = spokenLookup(parsed);
      let j = quote ? index + 1 : i + 1;
      while (j < lines.length && !lines[j].trim()) j++;
      const ctx = j < lines.length ? lines[j].trim() : '';
      const hasCtx = Boolean(ctx) && !/^\*\*/.test(ctx) && !parseRef(ctx) && !/^[“”"]/.test(ctx);
      if (canonical) {
        out.push(`**${canonical.citation}**`);
        out.push(`“${canonical.text}”`);
        if (hasCtx) out.push(ctx);
      }
      // Not His speech: the citation, whatever was quoted under it, and its gloss all leave the page.
      i = hasCtx ? j + 1 : (quote ? j : i + 1);
      continue;
    }
    out.push(lines[i]);
    i++;
...
const { CRISIS_NOTICE, looksLikeCrisis } = require('./crisis');
