...
// Narrator frames in which Jesus is the named speaker. Jesus never narrates
// himself in the third person inside a parable, so everything up to the comma
// after the speech verb is the evangelist's, not his.
const JESUS_FRAME_RE = /^(?:.{0,140}?\bJesus\b[^.;]*?\b(?:said|saith|answered|answereth|answering|spake|began to say|began to speak)\b[^,]*,\s*|.{0,80}?\b(?:said|saith|spake)\s+Jesus\b[^,]*,\s*)/i;

// Frames with a pronoun subject that sit outside any parable (checked by hand
// against the KJV text; a parable's own "and he said unto him" is left intact
// because there the frame is part of what Jesus spoke).
const SPEECH_INTROS = [
  ...
  /^And he looked round about on them which sat about him, and said,\s*/i,
  /^And he stretched forth his hand toward his disciples, and said,\s*/i,
  /^And he sighed deeply in his spirit, and saith,\s*/i,
  /^And he sat down, and called the twelve, and saith unto them,\s*/i,
  /^And he taught, saying unto them,\s*/i,
  /^And he called unto him his disciples, and saith unto them,\s*/i,
  /^And he sendeth forth two of his disciples, and saith unto them,\s*/i,
  /^And he began to speak unto them by parables\.\s*/i,
  /^And he lifted up his eyes on his disciples, and said,\s*/i,
  /^And he spake also a parable unto them;\s*/i,
  /^And he spake a parable unto them,\s*/i,
  /^And he turned to the woman, and said unto Simon,\s*/i,
  /^And he turned him unto his disciples, and said privately,\s*/i,
  /^But he, knowing their thoughts, said unto them,\s*/i,
  /^And when the people were gathered thick together, he began to say,\s*/i,
  /^And when the messengers of John were departed, he began to speak unto the people concerning John,\s*/i,
  /^Then he took unto him the twelve, and said unto them,\s*/i,
  /^And he beheld them, and said,\s*/i,
  /^And he said unto him,\s*/i,
  /^And he said unto her,\s*/i,
  /^And he answered and said unto them,\s*/i,
  /^And he answered them, saying,\s*/i,
  /^He answered and said unto them,\s*/i,
  /^He saith unto them,\s*/i,
  /^He said unto them,\s*/i,
  /^and (?:saith|said) unto (?:them|him|her),\s*/i,
  /^And the Lord said,\s*/i,
  /^The Lord then answered him, and said,\s*/i,
];

const SPEECH_TAILS = [
  /\s+And the wind ceased, and there was a great calm\.?$/i,
  /\s+And they said, Seven\.?$/i,
];
...
function extractSpoken(text, citation) {
  ...
  let spoken = cleaned;
  const framed = spoken.match(JESUS_FRAME_RE);
  if (framed && spoken.length - framed[0].length > 8) spoken = spoken.slice(framed[0].length);
...
// A marker the model half-typed ("{{John 14:27}", "{John 14:27}}") is read
// as the marker it meant, so it is filled or dropped like any other instead
// of leaking braces into the letter.
...
function fillPlaceholders(text) {
  if (!text) return text;
  const DROP = '\u0000DROP\u0000';
  const filled = normalizeMarkers(text).replace(PLACEHOLDER_RE, (_, raw) => {
...
// Every book of the Bible that is not one of the four Gospels. "1 John",
// "2 John" and "3 John" are listed so they are never read as the Gospel.
const OTHER_BOOKS = '(?:[123]\\s?John|Genesis|Exodus|Leviticus|Numbers|Deuteronomy|Joshua|Judges|Ruth|[12]\\s?Samuel|[12]\\s?Kings|[12]\\s?Chronicles|Ezra|Nehemiah|Esther|Job|Psalms?|Proverbs|Ecclesiastes|Song of (?:Solomon|Songs)|Isaiah|Jeremiah|Lamentations|Ezekiel|Daniel|Hosea|Joel|Amos|Obadiah|Jonah|Micah|Nahum|Habakkuk|Zephaniah|Haggai|Zechariah|Malachi|Acts|Romans|[12]\\s?Corinthians|Galatians|Ephesians|Philippians|Colossians|[12]\\s?Thessalonians|[12]\\s?Timothy|Titus|Philemon|Hebrews|James|[12]\\s?Peter|Jude|Revelation|Rom|Cor|Gal|Eph|Phil|Col|Thess|Tim|Heb|Pet|Rev|Gen|Exod|Lev|Deut|Josh|Judg|Sam|Chron|Neh|Esth|Ps|Prov|Eccl|Isa|Jer|Lam|Ezek|Dan|Hos|Mic|Nah|Hab|Zeph|Hag|Zech|Mal)';
const OTHER_REF_RE = new RegExp(`\\b${OTHER_BOOKS}\\.?\\s+\\d{1,3}(?::\\d{1,3})?(?:\\s*[–—\\-]\\s*\\d{1,3})?`, 'i');
const OTHER_BOLD_RE = new RegExp(`^\\*\\*\\s*${OTHER_BOOKS}\\.?\\s+\\d{1,3}(?::\\d{1,3})?(?:\\s*[–—\\-]\\s*\\d{1,3})?\\s*\\*\\*\\s*$`, 'i');

// A quotation typed into prose: six or more words between quotation marks.
const LONG_QUOTE_RE = /[“"]([^“”"]{2,}?)[”"]/g;
const QUOTE_WORDS_FLOOR = 6;
...
function longQuoteIn(sentence) {
  LONG_QUOTE_RE.lastIndex = 0;
  let m;
  while ((m = LONG_QUOTE_RE.exec(sentence))) {
    if (m[1].trim().split(/\s+/).length >= QUOTE_WORDS_FLOOR) return m[1];
  }
  return null;
}

// Prose lines are checked sentence by sentence. A sentence is removed when it
// cites another book, quotes narration, or types a quotation of its own; a
// bare Gospel reference without a quotation is left alone.
function auditProseLine(line, dropped) {
  const sentences = splitSentences(line);
  const kept = [];
  for (const s of sentences) {
    const other = s.match(OTHER_REF_RE);
    if (other) { dropped.push({ kind: 'other-author', text: s.trim() }); continue; }
    const refs = parseAllRefs(s);
    const narration = refs.find((r) => loadSpoken().books[r.book] && !isRedLetter(r));
    const quote = longQuoteIn(s);
    if (narration && quote) { dropped.push({ kind: 'narration', text: s.trim() }); continue; }
    if (quote) { dropped.push({ kind: 'typed-quote', text: s.trim() }); continue; }
    kept.push(s);
  }
  return kept.join('').replace(/\s+$/, '');
}

function skipContext(lines, j) {
  while (j < lines.length && !lines[j].trim()) j++;
  const ctx = j < lines.length ? lines[j].trim() : '';
  if (ctx && !/^\*\*/.test(ctx) && !parseRef(ctx) && !isQuoteLine(ctx)) j++;
  return j;
}

// Walks a letter and returns the text the page may show plus the list of
// quotations that were removed: citations to other books, to narration, and
// verses the model recited instead of marking. Bold Gospel citations are
// replaced with the recorded text.
function auditAdvisorText(text) {
  const dropped = [];
  ...
  while (i < lines.length) {
    const trimmed = lines[i].trim();
    if (OTHER_BOLD_RE.test(trimmed)) {
      const { quote, index } = extractQuotedLine(lines, i + 1);
      dropped.push({ kind: 'other-author', text: `${trimmed}${quote ? ` ${quote}` : ''}` });
      i = skipContext(lines, quote ? index + 1 : i + 1);
      continue;
    }
    ...
    if (looksLikeCite) {
      ...
      if (canonical && !canonical.redLetter && loadSpoken().books[parsed.book]) {
        ...
        dropped.push({ kind: 'narration', text: `${probe}${quote ? ` ${quote}` : ''}` });
        i = skipContext(lines, quote ? index + 1 : i + 1);
        continue;
      }
      if (canonical) {
        ...
        if (j < lines.length) {
          ...
          if (ctx && !/^\*\*/.test(ctx) && !parseRef(ctx) && !isQuoteLine(ctx)) {
            out.push(ctx);
            j++;
          }
        }
      ...
      // An unknown Gospel reference (chapter or verse that does not exist).
      dropped.push({ kind: 'unknown-ref', text: `${probe}${quote ? ` ${quote}` : ''}` });
      i = skipContext(lines, quote ? index + 1 : i + 1);
      continue;
    }
    // A quote line that does not follow a citation is a recited verse.
    if (isQuoteLine(trimmed) && trimmed.replace(/[“”"]/g, '').trim().split(/\s+/).length >= QUOTE_WORDS_FLOOR) {
      dropped.push({ kind: 'typed-quote', text: trimmed });
      i++;
      continue;
    }
    if (trimmed) {
      const audited = auditProseLine(lines[i], dropped);
      if (audited.trim()) out.push(audited);
      i++;
      continue;
    }
    out.push(lines[i]);
...
function verifyAdvisorText(text) {
  return auditAdvisorText(text).text;
}
...
// Detection lives in one module shared with the page (public/js/safety-patterns.js)
// so the server notice and the client modal can never disagree about a message.
const safety = require('../public/js/safety-patterns.js');

function looksLikeCrisis(text) {
  return safety.detectKind(text) === 'crisis';
}
...
function looksLikeDanger(text) {
  const kind = safety.detectKind(text);
  return kind === 'danger' || kind === 'assault';
}
...
function safetyKind(text) {
  return safety.detectKind(text);
}

// A disclosure stays in force for the turns that follow it: "should I forgive
// him and stay?" belongs to the same conversation as "my husband hits me".
function conversationSafety(messages) {
  return safety.detectConversation(messages);
}

function noticeForKind(kind) {
...
function safetyNotice(text) {
  return noticeForKind(safetyKind(text));
}
