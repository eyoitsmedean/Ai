...
const STOP = new Set([
  ...
  // Question and filler words that matched stray carry lines ("what I did").
  'what', 'did', 'does', 'say', 'said', 'says', 'about', 'jesus', 'someone', 'something',
  'want', 'how', 'who', 'keep', 'keeps', 'even', 'still', 'know', 'think', 'thought',
  'because', 'when', 'where', 'would', 'could', 'should', 'there', 'here', 'into',
]);
...
const GRAVITY = [
  ...
  [/\b(forgive|forgiving|forgiveness|forgave)\b/i, /Matthew 6:14/, 14],
  [/\b(afraid of the future|heart be troubled)\b/i, /John 14:1/, 16],
  [/\b(worry|worried|worrying|anxious|anxiety|overthinking)\b/i, /Matthew 6:34/, 14],
  [/\b(lonely|alone|loneliness|nobody left|everyone left)\b/i, /John 14:18/, 16],
  [/\b(revenge|get back at|pay(?:back)?|make (?:him|her|them) pay|my enemy)\b/i, /Matthew 5:44/, 16],
  [/\b(hate myself|sinning|keep sinning|disgusted with myself|forgive myself)\b/i, /John 8:11/, 18],
  // "I'm dying to see my grandkids" is an idiom, not a fear; "dying" alone never pulls.
  [/\b(scared of dying|afraid of dying|afraid of death|scared of death|fear of death|terminal|(?:i am|i'm|im) dying(?! to)|going to die soon|not long to live)\b/i, /John 11:25/, 16],
  [/\b(money|bills|rent|debt|afford|broke|paycheck|savings|mortgage|groceries)\b/i, /Matthew 6:25/, 16],
  [/\b(exhausted|so tired|worn out|burnt out|burned out|caregiver|caring for|no rest|cannot rest)\b/i, /Matthew 11:28/, 16],
  [/\b(far from god|distant from god|where is god|cannot find god|can't find god|god feels far|looking for jesus)\b/i, /Matthew 11:28/, 16],
  [/\b(?:my )?(?:child|son|daughter|baby|kid|little one) is (?:sick|ill|in the hospital|in hospital|dying)\b/i, /Mark 5:36/, 20],
  [/\b(feel lost|i am lost|so lost|feel like a failure|far from home|come home|coming home)\b/i, /Luke 15:20/, 16],
  [/\b(lied|i lie|lying|cheated|betrayed (?:him|her|them)|i hurt (?:him|her|them|someone))\b/i, /John 8:11/, 14],
];
...
function matchNeed(query, { limit = 6 } = {}) {
  ...
  const qPlain = qTokens.join(' ');
  const scored = [];
  for (const row of sealedNeeds()) {
    let score = 0;
    const hay = `${row.carry} ${row.theme} ${row.quote}`.toLowerCase();
    const carryPlain = tokens(row.carry).join(' ');
    const carryTokens = tokens(row.carry);
    if (hay.includes(q) || q.includes(row.carry.toLowerCase()) || (carryTokens.length >= 2 && qPlain.includes(carryPlain))) score += 20;
    // A question that restates most of a carry line ("I am not a Christian.
    // Can I still read this?") is that line, punctuation aside.
    if (carryTokens.length >= 2) {
      const hits = carryTokens.filter((t) => qSet.has(t)).length;
      const coverage = hits / carryTokens.length;
      if (coverage >= 0.8) score += 18;
...
// A single shared word scores 6–7; a real match scores in the twenties. Below
// the floor the honest answer is the standing letter, not the nearest page.
const NEED_FLOOR = 14;
function bestNeed(query, { minScore = 0 } = {}) {
  const hit = matchNeed(query, { limit: 1 })[0] || null;
