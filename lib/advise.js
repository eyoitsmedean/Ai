const { retrieveSayings, BETRAYED_RE } = require('./retrieve');
...
const BETRAYED_OPENING = 'The day you find out is not the day He asks you to forgive. He sits with the wound first.';
...
const BETRAYED_PASSAGES = [
  { verse: 'Matthew 11:28', context: 'The invitation is to the exhausted. Rest is a gift, not a prize for the strong.' },
  { verse: 'Matthew 5:4', context: 'What was broken is a loss. Comfort is promised to those who actually mourn it.' },
  { verse: 'John 14:27', context: 'He leaves peace the way someone leaves a key. It is already in the house.' },
];
const DANGER_PASSAGES = [
  { verse: 'Matthew 11:28', context: 'The invitation is to the exhausted. Rest is a gift, not a prize for the strong.' },
  { verse: 'John 14:27', context: 'He leaves peace the way someone leaves a key. It is already in the house.' },
  { verse: 'Matthew 5:4', context: 'Comfort is promised to those who actually mourn — not to those who pretend the loss was small.' },
];
const BY_YOU_PASSAGES = [
  { verse: 'Luke 15:4', context: 'He does not wait for you to find the road back. He comes after what is lost.' },
  { verse: 'Matthew 11:28', context: 'The invitation is to the exhausted. Rest is a gift, not a prize for the strong.' },
  { verse: 'Luke 12:7', context: 'Counted, down to the hairs of your head. You are not a burden to Him — and you are still responsible for what you do next.' },
];
...
function roomFor(question) {
  ...
  if (!q) {
    const pack = encouragementFor('Peace');
    return {
      question: '',
      crisis: false,
      bereaved: false,
      ...
      primary: 'Peace',
      pack,
      opening: 'Peace I leave with you.',
      allowed: ['Matthew 11:28', 'John 14:27'],
    };
  }
  ...
  const betrayed = !crisis && !danger && BETRAYED_RE.test(q);
  ...
  if (crisis && bereaved) {
    ...
    allowed = [...CRISIS_SAFE];
  } else if (danger && byYou) {
    primary = 'Shame & Guilt';
    opening = DANGER_BY_YOU_OPENING;
    allowed = [...BY_YOU_PASSAGES.map((p) => p.verse), ...encouragementFor(primary).passages.map((p) => p.verse)];
  } else if (danger) {
    primary = 'Suffering & Pain';
    opening = DANGER_OPENING;
    allowed = [...DANGER_PASSAGES.map((p) => p.verse), ...encouragementFor('Peace').passages.map((p) => p.verse)];
  } else if (betrayed) {
    primary = 'Suffering & Pain';
    opening = BETRAYED_OPENING;
    allowed = [...BETRAYED_PASSAGES.map((p) => p.verse), ...encouragementFor('Grief & Loss').passages.map((p) => p.verse)];
  } else {
    ...
    if (themes.length) {
      ...
      allowed = ['Matthew 11:28', 'John 14:27', 'Matthew 28:20', 'John 14:18'];
    }
  }
  const pack = encouragementFor(primary) || encouragementFor('Peace');
  return { question: q, crisis, bereaved, danger, byYou, betrayed, themes, primary, pack, opening, allowed: [...new Set(allowed)] };
}
...
function adviseLetter(question) {
  const q = String(question || '').trim();
  if (!q) {
    // Empty send is two calm lines — never the day's word, which can be tribulation.
  ...
  const { crisis, bereaved, danger, byYou, betrayed, themes, pack, opening } = room;

  let chosen;
  if (crisis && !bereaved) {
    chosen = CRISIS_PASSAGES.map((p) => ({ cite: p.verse, context: p.context }));
  } else if (danger && byYou) {
    chosen = BY_YOU_PASSAGES.map((p) => ({ cite: p.verse, context: p.context }));
  } else if (danger) {
    chosen = DANGER_PASSAGES.map((p) => ({ cite: p.verse, context: p.context }));
  } else if (betrayed) {
    chosen = BETRAYED_PASSAGES.map((p) => ({ cite: p.verse, context: p.context }));
  } else {
    chosen = [];
    const seen = new Set();
    const add = (p) => {
    ...
    if (noRoom) {
      add({ verse: 'Matthew 11:28', context: 'The invitation is to the exhausted, not the already-healed.' });
      add({ verse: 'John 14:27', context: 'Peace is left with you — a gift, not a mood you manufacture.' });
    } else {
