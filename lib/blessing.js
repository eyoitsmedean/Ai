...
function blessingPage({ verse, quote, note }, origin) {
  ...
  const roomHref = '/';
  // Link previews should show the sentence, not the chrome: title is the verse,
  // description is His words, and the note (if any) follows.
  const title = `${verse} — a blessing`;
  const desc = (`“${quote}”` + (note ? ` — ${note}` : '')).slice(0, 200);
...
<head>
  ...
  <meta property="og:site_name" content="Red Letter" />
  <meta name="twitter:card" content="summary" />
  <meta name="twitter:title" content="${escapeHtml(title)}" />
  <meta name="twitter:description" content="${escapeHtml(desc)}" />
  ${canon ? `<meta property="og:url" content="${escapeHtml(canon)}" />` : ''}
  ${origin ? `<meta property="og:image" content="${escapeHtml(origin.replace(/\/$/, ''))}/icon-512.png" />` : ''}
