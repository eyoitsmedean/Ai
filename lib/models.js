...
// GPT-6 Astra returns HTTP 400 for `none`, so it is deliberately absent.
const OPENAI_REASONING_EFFORTS = ['low', 'medium', 'high', 'xhigh', 'max'];

function reasoningEffort(value) {
  if (!value) return null;
  const effort = String(value).trim().toLowerCase();
  if (OPENAI_REASONING_EFFORTS.includes(effort)) return effort;
  console.warn(`Ignoring OPENAI_REASONING_EFFORT=${value}; use one of ${OPENAI_REASONING_EFFORTS.join(', ')}.`);
  return null;
}
...
function openaiProvider(model, env) {
  ...
  const effort = reasoningEffort(env.OPENAI_REASONING_EFFORT);
  // Readers bring private struggles here; do not let OpenAI retain the exchange
  // in its stored-response history (the Responses API keeps it 30 days by default).
