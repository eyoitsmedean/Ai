...
const THEMES = {
  ...
  Forgiveness: {
    ...
    passages: [
      passage('Matthew 6:14', 'The same mercy you release is the mercy that keeps finding you.'),
      passage('Luke 6:36', 'Mercy is not a personality trait. It is family resemblance.'),
      passage('Luke 6:27', 'Love of enemies is not sentiment. It is a practice that begins while the hurt is still warm.'),
    ],
  ...
  'Faith & Doubt': {
    ...
    passages: [
      ...
      passage('Mark 11:24', 'Prayer is not a wish. It is asking as if the Father already knows how to give.'),
...
const DAILY_ROTATION = [
  ...
  },
  {
    ...
    word: {
      ...
      verse: 'John 14:27',
      reflection: 'If your heart is troubled, that is not evidence that peace has failed. It is the exact condition He speaks into.',
    },
  },
  {
    affirmation: {
...
function dailyForDate(date = new Date()) {
  const idx = Math.floor(date.getTime() / 86400000) % DAILY_ROTATION.length;
