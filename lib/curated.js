...
const THEMES = {
  'Anxiety & Worry': {
    ...
    more: [
      passage('Matthew 6:27', 'Worry has never once lengthened a life. He says so plainly, without scolding.'),
      passage('Matthew 11:29-30', 'The yoke is easy because it is shared. Rest is the promise, not the prerequisite.'),
    ],
  ...
  'Grief & Loss': {
    ...
    more: [
      passage('John 16:20', 'He does not skip the weeping. He tells you where it ends.'),
      passage('John 14:2', 'A place is being prepared for you, in a house you have not seen.'),
    ],
  ...
  Forgiveness: {
    ...
    more: [
      passage('Matthew 18:22', 'Seventy times seven is not arithmetic. It is a way of not keeping count.'),
      passage('Luke 23:34', 'He forgave while the harm was still happening. The road begins before the apology.'),
    ],
  ...
  Loneliness: {
    ...
    passages: [
      ...
      passage('John 14:23', 'Not a visit. An abode. He speaks of moving in, not dropping by.'),
    ],
    practice: 'Place a chair across from you. Read John 14:18 as if it were spoken from that chair. Sit with it for one minute.',
    more: [
      passage('Matthew 18:20', 'Two or three is enough for His presence. So is one.'),
      passage('John 15:15', 'Not servant. Friend. The word is His, not yours to earn.'),
    ],
  ...
  'Conflict & Relationships': {
    ...
    more: [
      passage('Matthew 5:9', 'Peacemaking is a family trait, not a personality.'),
      passage('John 13:34', 'The measure of love is His, not the other person’s behaviour. You are not asked to earn it back.'),
    ],
  ...
  Fear: {
    ...
    more: [
      passage('Matthew 14:27', 'The storm is real. So is the voice inside it.'),
      passage('Matthew 10:31', 'Your value is the argument he makes against fear.'),
    ],
  ...
  'Purpose & Direction': {
    ...
    more: [
      passage('Matthew 4:19', 'The call is one word long. The making is His.'),
      passage('John 15:16', 'You were chosen before you chose. The fruit is not your invention.'),
    ],
  ...
  'Faith & Doubt': {
    ...
    more: [
      passage('Mark 9:23', '“If thou canst” is allowed in the room. He answers a maybe with a promise.'),
      passage('Luke 22:32', 'Before the failure, He had already prayed for the one who would fail. Your faith is being kept for you.'),
    ],
  ...
  'Suffering & Pain': {
    ...
    more: [
      passage('Matthew 11:29-30', 'The burden is light because it is carried with you.'),
      passage('Luke 6:21', 'Weeping is blessed, not diagnosed. Laughter is spoken over it as a promise, not a demand.'),
    ],
  ...
  'Shame & Guilt': {
    ...
    more: [
      passage('John 8:11', 'No condemnation is the first word. The next step is the second.'),
      passage('Luke 19:10', 'Lost is the condition He came for, not the one He avoids.'),
    ],
  ...
  Peace: {
    ...
    more: [
      passage('Matthew 11:29', 'Rest unto your souls — the peace is placed where the trouble is.'),
      passage('John 14:1', 'A troubled heart is spoken to, not shamed.'),
    ],
  ...
  Hope: {
    ...
    more: [
      passage('John 14:2-3', 'A place is prepared. Hope has an address.'),
      passage('Luke 12:32', 'The gift is a kingdom. The receiver is a little flock.'),
    ],
...
const COMMONS = [
  passage('Matthew 11:28', 'The invitation is to the exhausted, not the already-healed.'),
  passage('John 14:27', 'Peace is left with you — a gift, not a mood you manufacture.'),
  passage('Matthew 7:7', 'Asking is allowed. Seeking is allowed. The door is His to open, and He says it opens.'),
  passage('John 6:37', 'Coming is enough. He does not turn away the one who comes.'),
  passage('John 10:27-28', 'You are known by name, and held by a hand that does not open.'),
  passage('Mark 4:39', 'The storm is addressed by name. Calm is spoken, not negotiated.'),
];
...
function encouragementFor(theme) {
  ...
  const pack = THEMES[resolveTheme(theme) || theme];
  if (!pack) return null;
  const { more, ...rest } = pack;
  return { ...rest, translation: 'KJV', verified: true, source: 'curated' };
}
...
function dailyRotation() {
  return DAILY_ROTATION.map(hydrateDaily);
}

// Named paths. Seven is the first week; MORE_DAYS continues it (the longer story is not yet built).
// Only the reference, title and reflection are authored; the words come from the corpus.
function day(title, theme, verse, reflection) {
  const hit = lookup(verse);
  if (!hit) throw new Error(`Missing path verse: ${verse}`);
  return { title, theme, verse: hit.citation, passage: hit.text, reflection };
}

const SEVEN = [
  day('Come', 'Rest', 'Matthew 11:28-29', 'Day one is not a program. It is an invitation. Come as you are — laden, not finished.'),
  day('Peace', 'Peace', 'John 14:27', 'The world offers a pause. He leaves a gift. You do not have to manufacture calm to receive it.'),
  day('Light', 'Light', 'John 8:12', 'Dark seasons are real. He does not deny them. Following is how the next step becomes visible.'),
  day('Love', 'Love', 'John 13:34', 'The mark is not an argument. It is how you treat the person next to you today.'),
  day('Forgive', 'Forgiveness', 'Matthew 18:22', 'Mercy is a way of life, not a single heroic act. One name is enough for this day.'),
  day('Abide', 'Abide', 'John 15:4-5', 'Fruit comes from staying close, not from straining alone. Remain. That is the work.'),
  day('Go', 'Presence', 'Matthew 28:19-20', 'The last word of the seven is not goodbye. It is presence that does not expire. Go — He goes too.'),
];

const MORE_DAYS = [
  day('Resurrection', 'Life', 'John 11:25-26', 'He does not offer a theory of death. He offers himself in the middle of it.'),
  day('Bread', 'Hunger', 'John 6:35', 'Hunger is honest. He does not scold it. He names himself as enough.'),
  day('Shepherd', 'Care', 'John 10:11', 'Care that costs nothing is a hireling. He stays when it is expensive.'),
  day('Troubled', 'Peace', 'John 14:1', 'He speaks to a room about to break. Belief here is company, not a quiz.'),
  day('Overcome', 'Courage', 'John 16:33', 'He tells the truth about the world first. Cheer is not denial.'),
  day('Paradise', 'Mercy', 'Luke 23:43', 'A dying thief is given today, not a waitlist. Mercy does not arrive late.'),
  day('Peacemakers', 'Peace', 'Matthew 5:9', 'Peace is made, not merely felt. One reconciling step is enough for this day.'),
  day('Anointed', 'Purpose', 'Luke 4:18-19', 'His purpose statement is toward the poor, the broken, the locked. That is the work.'),
  day('Sent', 'Go', 'John 20:21', 'Peace first. Then a sending. You do not go unaccompanied.'),
  day('Willing', 'Surrender', 'Matthew 26:39', 'He does not pretend the cup is sweet. Honesty and obedience can sit in one sentence.'),
  day('Ninety-nine', 'Seeking', 'Luke 15:4-5', 'You are not an acceptable loss. He goes after the one, and He is glad when He finds you.'),
];
