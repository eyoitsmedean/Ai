...
// Filled KJV letters for the PWA when /api/chat cannot be reached after a
// safety modal. Same wording and substitution as finishWithLetter on the
// server. The model never writes these sentences.
function buildSafetyPack() {
  return {
    version: 1,
    translation: 'KJV',
    kinds: {
      crisis: {
        ...
        letter: verifyAndSubstitute(CRISIS_LETTER),
        followup: verifyAndSubstitute(CRISIS_FOLLOWUP_LETTER),
      },
      danger: {
        notice: DANGER_NOTICE,
        letter: verifyAndSubstitute(DANGER_LETTER),
        followup: verifyAndSubstitute(DANGER_FOLLOWUP_LETTER),
      },
      assault: {
        notice: ASSAULT_NOTICE,
        letter: verifyAndSubstitute(ASSAULT_LETTER),
        followup: verifyAndSubstitute(DANGER_FOLLOWUP_LETTER),
      },
      greeting: {
        notice: '',
        letter: verifyAndSubstitute(GREETING_LETTER),
        followup: verifyAndSubstitute(GREETING_LETTER),
      },
    },
  };
}
