...
/**
 * Days since the nearest Advent Sunday (0 on Advent Sunday itself).
 * January dates count from the previous year's Advent so a 28-room path
 * that started in late November can still be finished after Christmas.
 * Negative before Advent; keeps counting past 27 so callers can clamp.
 */
function adventDayIndex(date) {
  const d = date instanceof Date ? date : new Date(date || Date.now());
  const year = d.getMonth() < 6 ? d.getFullYear() - 1 : d.getFullYear();
  const start = adventSunday(year);
...
/**
 * Days since Ash Wednesday of that civil year (0 on Ash Wednesday).
 * Negative before Lent; keeps counting past 39 so callers can clamp
 * a 40-room path to the last room through Holy Saturday.
 */
function lentDayIndex(date) {
  const d = date instanceof Date ? date : new Date(date || Date.now());
  const start = ashWednesday(d.getFullYear());
  const a = Date.UTC(d.getFullYear(), d.getMonth(), d.getDate());
  const b = Date.UTC(start.getFullYear(), start.getMonth(), start.getDate());
  return Math.round((a - b) / 86400000);
}
