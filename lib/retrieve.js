...
const NEED_CUES = [
  ...
  [/\b(grief|grieving|grieve|mourn|died|death|funeral|widow)\b/i, 'Grief & Loss'],
  [/\b(forgiv|resent|bitter|hate them|trespass)\b/i, 'Forgiveness'],
  [/\b(lonely|loneliness|alone|abandon|orphan|left me)\b/i, 'Loneliness'],
...
function retrieveSayings(query, opts = {}) {
  const { limit = 8 } = opts;
  ...
  if (!picks.length) {
    if (opts.allowEmpty) {
      return { themes, sayings: [], unmatched: true };
    }
    return {
      themes,
