...
const NEED_CUES = [
  [/\b(weary|heavy[- ]laden|exhausted|burn(?:ed|t)? ?out)\b/i, 'Suffering & Pain'],
  [/\b(anxi|worr|overwhelm|stress|tomorrow|panic|restless|tired)\b/i, 'Anxiety & Worry'],
...
const COMFORT_REFS = ['John 14:27', 'Matthew 11:28', 'Mark 4:39'];
...
function isComfortSaying(saying) {
  return COMFORT_REFS.some((cite) => sayingTouchesCitation(saying, cite));
}

function comfortSayings(limit = 3) {
  return loadLibrary().sayings.filter(isComfortSaying).slice(0, limit);
}
