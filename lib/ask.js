...
const CANNOT_ABUSE = [
  'This page is not a pastor, not a shelter, and not a substitute for an advocate who can help you plan how to be safe.',
  'If you are in immediate danger, call 911. United States: 1-800-799-7233, text START to 88788, or thehotline.org. Sexual assault: 800-656-4673, text HOPE to 64673.',
].join('\n');
...
const HANDOFF = {
  ...
  crisisLoss: [
    ...
    '',
    '988 (call, text, or chat at 988lifeline.org) also answers for people grieving a death by suicide or overdose. People who have lost someone this way can themselves be at risk; the number is for you tonight.',
    'Anywhere else: https://findahelpline.com/',
    '',
  ...
  abuse: [
    ...
    '',
    'This page will not add a verse. I am not a person, and this page cannot keep anyone safe.',
  ].join('\n'),
  abuseCrisis: [
    'What you are describing is violence, and you also named danger to a life. If anyone is in danger right now, call 911 or your local emergency number.',
    '',
    'United States: National Domestic Violence Hotline — 1-800-799-7233, text START to 88788, or thehotline.org.',
    'Sexual assault: RAINN — 800-656-4673, text HOPE to 64673, or hotline.rainn.org.',
    'If you are thinking of ending your life: 988 — call, text, or chat. https://988lifeline.org/',
    '',
    'If someone else can see this screen or this history, call 1-800-799-7233 instead of chatting. This page cannot hide a visit. Clear the history if you can.',
    '',
    'This page will not add a verse. I am not a person, and this page cannot keep anyone safe.',
  ...
  spanishCrisis: [
    ...
    '',
    'Estados Unidos: 988, luego 2 para español; o envía AYUDA al 988; o chat en español. https://988lifeline.org/',
...
function priorStop(prior) {
  ...
  for (const line of prior) {
    const t = String(line || '');
    if (ABUSE_RE.test(t) && (looksLikeCrisis(t) || SPANISH_CRISIS_RE.test(t))) return 'abuseCrisis';
    if (SPANISH_CRISIS_RE.test(t)) return 'spanishCrisis';
    const kind = stopKind(classify(t));
    if (kind) return kind;
  }
  return '';
}
...
function cannotFor(kind) {
  return kind === 'abuse' || kind === 'abuseCrisis' ? CANNOT_ABUSE : CANNOT;
}
...
function composeAsk(raw, { prior = [] } = {}) {
  ...
  /* classify() ranks crisis before abuse (folio contract). /ask must not
     drop NDVH when both fire, including when the second line adds the other danger. */
  if (ABUSE_RE.test(text) && (looksLikeCrisis(text) || SPANISH_CRISIS_RE.test(text))) {
    return stopResult('abuseCrisis');
  }
  const held = priorStop(prior);
  if (held) {
    const heldCrisis = held !== 'abuse' && held !== 'abuseCrisis';
    const heldAbuse = held === 'abuse' || held === 'abuseCrisis';
    if (heldCrisis && ABUSE_RE.test(text)) return stopResult('abuseCrisis');
    if (heldAbuse && (looksLikeCrisis(text) || SPANISH_CRISIS_RE.test(text))) return stopResult('abuseCrisis');
    return stopResult(held);
  }
  if (SPANISH_CRISIS_RE.test(text)) return stopResult('spanishCrisis');
