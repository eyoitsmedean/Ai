...
const ASK = fs.readFileSync(path.join(ROOT, 'lib/ask.js'), 'utf8');
...
for (const [book, chs] of Object.entries(spoken.books)) {
  for (const [ch, verses] of Object.entries(chs)) {
    for (const vs of Object.keys(verses)) {
      const cite = `${book} ${ch}:${vs}`;
      RED.push(cite);
      if (!VERSES[cite]) {
        const hit = lookup(cite);
        if (hit && hit.redLetter && hit.text) VERSES[cite] = hit.text;
      }
    }
  }
}
...
const askBody = ASK
  .replace(/^const \{ classify \} = require\('\.\/advise'\);\n/m, '')
  .replace(/^const \{ lookup, looksLikeCrisis \} = require\('\.\/scripture'\);\n/m, '')
  .replace(/^const \{ THEMES \} = require\('\.\/curated'\);\n/m, '')
  .replace(/^const \{ tokens \} = require\('\.\/retrieve'\);\n/m, '')
  .replace(/^const ABUSE_RE = .*\n/m, '')
  .replace(/^const SPANISH_CRISIS_RE = .*\n/m, '')
  .replace(/\nmodule\.exports = \{[\s\S]*};\n?$/, '\n');
if (!/function composeAsk/.test(askBody)) {
  throw new Error('could not strip lib/ask.js for the on-device bundle');
}
